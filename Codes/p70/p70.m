%% The EIF demo with bearing & range sensor models.
...
path(path, genpath('../p57'));  % For calling sensor model
path(path, '../p69');           % For calling EIF functions
...
for i = 1 : step
    ...
    updateStart = tic;
    [mu, Omega, xi] =...
        updateEIF(mu, Omega, xi, @p57.sensorModel, sensorPos,...
        zTrue(:,i), H, R, true, 1);
...
%%
if enableVis 
    h = p70InitFigure(xTrue, muPost, sensorPos);
    if makeVideo
        videoObj           = VideoWriter('bearingrangeeifdemo.avi');
